...
class LitePreKeyStore(PreKeyStore):
    ...
    def loadUnsentPendingPreKeys(self) -> Any:
    ...
    def setAsSent(self, prekeyIds) -> Any:
        """
        :param preKeyIds:
        :type preKeyIds: list
        :return:
    ...
    def storePreKeyBatch(self, preKeyRecords) -> Any:
        """
        Batch store multiple prekeys with a single commit for better performance.
        :param preKeyRecords: list of tuples (preKeyId, preKeyRecord)
        :type preKeyRecords: list
        """
        q = "INSERT INTO prekeys (prekey_id, record) VALUES(?,?)"
        cursor = self.dbConn.cursor()
        for preKeyId, preKeyRecord in preKeyRecords:
            serialized = preKeyRecord.serialize()
            cursor.execute(q, (preKeyId, buffer(serialized) if sys.version_info < (2,7) else serialized))
        self.dbConn.commit()
